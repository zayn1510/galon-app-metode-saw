...
export default function KecamatanCard({ data,refreshData,message,setMessage,currentPage,itemsPerPage,onPageChange }:Props) {
    const [loading, setLoading] = useState(true);
    const [selectedKecamatan, setSelectedKecamatan] = useState<KecamatanResource | null>(null);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [isModalConfirm, setIsModalConfirm] = useState(false);
    
    const [itemToDelete, setItemToDelete] = useState<KecamatanResource | null>(null);
    const startIndex = (currentPage - 1) * itemsPerPage;

    
    
    const handleEdit = async (item: KecamatanResource) => {
      ...
      <div className="fixed inset-0 bg-gray-500 bg-opacity-50 flex justify-center items-center z-50">
        <div className="bg-white p-6 rounded-lg shadow-lg w-96">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Apakah Anda yakin ingin menghapus Kecamatan: <strong>{itemToDelete.nama_kecamatan}</strong>?
          </p>
  ...
  const handleDelete = async (item: KecamatanResource) => {
    try {
        ...
        if (!res.ok) {
            setMessage({
                text:"Gagal menghapus Kecamatan",
                status:false
            });
        }
        if (result.status) {
...
if (loading) {
  ...
  const handleConfirmDelete = (item: KecamatanResource) => {
...
return (
  ...
  return (
    <div className="overflow-x-auto mt-6">
      {isModalOpen && selectedKecamatan && (
 <KecamatanModal setMessage={setMessage}
        ...
        <tbody>
          {data.map((item, index) => (
            <tr key={item.id} className="hover:bg-gray-50 transition">
              ...
              <td className="px-4 py-2 border-b">{item.nama_kecamatan}</td>
